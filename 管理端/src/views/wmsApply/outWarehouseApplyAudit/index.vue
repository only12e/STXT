          ...
          <el-table-column label="出库数量" prop="outQuantity" width="150">
            <template slot-scope="scope">
              <el-form-item :prop="'wmsOutWarehouseApplyDetailList.'+scope.$index+'.outQuantity'" :rules="rules.outQuantity">
                <el-input v-model.number="scope.row.outQuantity" placeholder="请输入出库数量" :disabled="form.auditStatus!='1'&&form.applyStatus!='1'?false:true" />
              </el-form-item>
            </template>
          </el-table-column>
...
<script>
import { listOutWarehouseApply4Audit, getOutWarehouseApply, auditOutWarehouseApply } from "@/api/wmsApply/outWarehouseApply";
